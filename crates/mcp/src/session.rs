...
impl Session {
    ...
    pub fn set_theme(
        ...
        if let Some(primary) = primary {
            project.theme.primary = ColorToken::parse(primary)
                .ok_or_else(|| format!("no palette colour called {primary}"))?;
        }
...
fn prop_from_json(
    ...
    match ty {
        ...
        PropType::Text | PropType::MultilineText => Ok(PropValue::Text(
        ...
        PropType::Choice => {
            let text =
                text.ok_or_else(|| format!("`{key}` wants one of {}", choices.join(", ")))?;
...
fn edges(value: &Value) -> Result<Edges, String> {
    ...
    let object = value.as_object().ok_or_else(|| {
        "spacing wants a number, or an object of top/right/bottom/left".to_string()
    })?;
    let side = |key: &str| {
        object
            .get(key)
